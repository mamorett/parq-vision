...
2. **`prompt`**: Configured as `"prompt"`. This field stores the actual prompt instruction sent to the Vision LLM (e.g. `"describe the image in detail"`).
...
Here is the complete list of all **6** supported configuration field types:
...
#### 2. `prompt`
- **Resulting Parquet Schema Type:** `STRING` (or `BYTE_ARRAY` with UTF-8 logical type)
- **Behavior:** Automatically populated with the actual prompt instruction sent to the Vision LLM (e.g. `"describe the image in detail"`).
- **Best Use Case:** Storing the model instructions used for that run directly inside the database row for record-keeping.
...
**Note**: `image_path` is always automatically included as the primary key (`STRING` type) and will store the **full absolute path** of the processed image file (e.g. `/path/to/my/images/ComfyUI_00006_.png`), resolved by the file collector.
